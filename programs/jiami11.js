// ========== character library definitions ==========
        ...
        // ========== v11 complete encryption flow ==========
        // 0.CBCXOR chain → 1.SHA-95reversible mixing → 2.bit encoding → 3.Triple rotating libraries → 4.position shuffle → 5.chained avalanche → 6.column transposition → 7.Perlin noise offset → 8.reverse
        function encryptV11(text, numKey, textKey, noiseKey, shaKey, cbcKey, initialStateCode) {
            // Step 0: CBCXOR chain
        ...
        // ========== v11 complete decryption flow ==========
        // implementation note
        function decryptV11(ciphertext, textKey, numKey, noiseKey, shaKey, cbcKey, stateCode) {
            // Step 8 inverse:reverserestore
            const unreversed = ciphertext.split('').reverse().join('');

            // implementation note
            const denoised = perlinNoiseDecrypt(unreversed, noiseKey);

            // Step 6 inverse:column transpositionrestore
            const untransposed = columnTransposeReverse(denoised, textKey);

            // Step 5 inverse:chained avalancherestore
            const unavalanched = chainAvalancheReverse(untransposed);

            // Step 4 inverse:positionrestore
            const unshuffled = unshufflePositions(unavalanched);

            // Step 3 inverse:Triple rotating librariesDecrypt
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('encrypt-btn').addEventListener('click', function() {
                ...
                try {
                    const result = encryptV11(plaintext, numKey, textKey, noiseKey, shaKey, cbcKey, initialStateCode);
                    document.getElementById('encrypted-result').textContent = result.ciphertext;
                    document.getElementById('key-info-result').textContent = 'Library state code: ' + result.stateCode + ' | CBC key: ' + cbcKey + ' | SHA key: ' + shaKey + ' | Numeric key: ' + numKey + ' | Text key: ' + textKey + ' | Noise seed: ' + noiseKey;
            ...
            document.getElementById('decrypt-btn').addEventListener('click', function() {
                ...
                try {
                    const result = decryptV11(ciphertext, textKey, numKey, noiseKey, shaKey, cbcKey, stateCode);
