// ========== character library definitions ==========
        ...
        // ========== v9 complete encryption flow ==========
        // 0.bit encoding → 1.Triple rotating libraries → 2.position shuffle → 3.chained avalanche → 4.column transposition → 5.Perlin noise offset → 6.reverse
        function encryptV9(text, numKey, textKey, noiseKey, initialStateCode) {
            // Step 0: bit encodinglayer
            const bitEncoded = bitEncode(text);

            // Step 1: Triple Rotating Library Encryption
            const tripleResult = tripleLibEncrypt(bitEncoded, numKey, initialStateCode);
            
            // Step 2: position shuffle
        ...
        // ========== v9 complete decryption flow ==========
        // implementation note
        function decryptV9(ciphertext, textKey, numKey, noiseKey, stateCode) {
            // Step 6 inverse:reverserestore
            ...
            // Step 2 inverse:positionrestore
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('encrypt-btn').addEventListener('click', function() {
                ...
                try {
                    const result = encryptV9(plaintext, numKey, textKey, noiseKey, initialStateCode);
                    document.getElementById('encrypted-result').textContent = result.ciphertext;
                    document.getElementById('key-info-result').textContent = 'Library state code: ' + result.stateCode + ' | Numeric key: ' + numKey + ' | Text key: ' + textKey + ' | Noise seed: ' + noiseKey;
            ...
            document.getElementById('decrypt-btn').addEventListener('click', function() {
                ...
                try {
                    const result = decryptV9(ciphertext, textKey, numKey, noiseKey, stateCode);
